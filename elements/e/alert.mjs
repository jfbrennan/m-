  ...
  return html`
    <style scope=global>
      ...
      e-alert {
        ...
        /* Dismiss button */
        & e-button[type=remove]:last-of-type { margin-left: auto }

        /* Types */
        &[type=info] {
          background-color: var(--e-color-blue-1);

          &::before { color: var(--e-color-blue-3) }
        }

        &[type=success] {
          background-color: var(--e-color-green-1);

          &::before { color: var(--e-color-green-3) }
        }

        &[type=warn] {
          background-color: var(--e-color-orange-1);

          &::before { color: var(--e-color-orange-3) }
        }

        &[type=error] {
          background-color: var(--e-color-red-1);

          &::before { color: var(--e-color-red-3) }
        }
      }
    </style>

    <slot></slot>
    ${dismissible ? '<e-button type=remove aria-label="Dismiss Alert" ></e-button>' : ''}
